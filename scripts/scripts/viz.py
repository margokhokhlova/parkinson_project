...
def plot_probabilistic_output(outputs,save_path,feat1_name='f1', feat2_name='f2'):
	#min_accuracy= np.min(outputs)
	#max_accuracy= np.max(outputs)
	...
	max_accuracy= 1
	# Example data
	xx, yy = np.meshgrid(np.linspace(min_accuracy, max_accuracy, 100), np.linspace(min_accuracy, max_accuracy, 100))
	...
	# plt.contourf(xx, yy, outputs, levels=[0.5 + i/100 for i in range(51)], cmap='coolwarm', alpha=0.6)  # Use 'coolwarm' cmap for red to blue gradient
	plt.colorbar()
	plt.xlabel(feat1_name)
	plt.ylabel(feat2_name)
...
def viz_quantity_features(X_features, label, model, save_path=None):
	import matplotlib.pyplot as plt
...
def plot_curve(vector, line, Title, label, save_path):
	# print(Title,": ",vector)
	plt.figure()
	BestValue= np.max(vector)
	RecentValue= vector[len(vector)-1]
	print("Recent ",label,": ",RecentValue)
	plt.title(f"{Title} (the best: {BestValue:.3f}, recent: {RecentValue:.3f})")
	plt.plot(vector, line, marker="o")
	new_path= save_path + "/" + label
	# print(new_path)
	plt.savefig(new_path)
	plt.clf()

def plot_std_curve(vector, line, Title, label, save_path):
	# print(Title,": ",vector)
	plt.figure()
	BestValue= np.min(vector)
	RecentValue= vector[len(vector)-1]
	print("Recent ",label,": ",RecentValue)
	plt.title(f"{Title} (the best: {BestValue:.3f}, recent: {RecentValue:.3f})")
	plt.plot(vector, line, marker="o")
...
def plot_two_curves(vector1, vector2, line1, line2, Title, label1, label2, save_path):
	# print(Title,": ",vector)
	plt.figure()
	BestValue1= np.max(vector1)
	BestValue2= np.max(vector2)
	plt.title(f"{Title} (the best: {BestValue1:.3f}, {BestValue2:.3f})")
	plt.plot(vector1, line1, marker="o", label=label1)
	plt.plot(vector2, line2, marker="o", label=label2)
	plt.legend()
	new_path= save_path + "/" + label1 + "_vs_" + label2
	# print(new_path)
	plt.savefig(new_path)
	plt.clf()
