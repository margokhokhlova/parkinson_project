...
def main():
    for model_features in all_combinations : 
        # to log all the experiments
        sys.stdout = open('/home/margokat/Documents/projects/margo_files/rocket/results/result_rocket.txt', 'a')
        # parameters
        min_len_established = 600
        num_val_cycles = 100
        #model_features= [6,0,4,3,1,5,8,11,13]
        ...
        for X_trainval, y_trainval, X_test, y_test, (i, j) in leave_one_pair_out(PDL_features, ETL_features):
            ...
            if (   
                np.isnan(X_val_transformed.values).any() or      
                np.isnan(X_train_transformed.values).any() or
                np.isinf(X_train_transformed.values).any() or
                len(np.unique(y_trainval)) < 2
            ):
                print("Skipping cycle due to invalid data (None, NaN, Inf, or not enough classes).")
                continue
        ...
        try :    
            ...
            if test_accuracy is not None and len(test_accuracy) > 0:
                print(f"Rocket, nkernels is {num_kernels}, combination {model_features} "
                f"Test Accuracy No Nans mean: {np.nanmean(test_accuracy)}, {np.nanstd(test_accuracy)}")
                print(test_accuracy, "\n")
            else:
                print("Skipping final reporting due to missing or empty test_accuracy.")
        print(test_accuracy, "\n")
