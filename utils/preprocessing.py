...
def preprocess_signal(original_signal, SamplingRate=500, LF=60, HF=240, frequences_to_filter = [50, 100, 150, 200], order_butter=4, save_plot=False):
    ...
    processed = butter_bandpass_filter(notch_filtered, LF,  HF, SamplingRate, order_butter)
    # evelope
    ...
    if save_plot:
        ...
        plt.figure(2)
        plt.clf()
        plt.magnitude_spectrum(original_signal, Fs=SamplingRate, label='Noisy signal magnitude')
        plt.axis('tight')
